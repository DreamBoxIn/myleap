// src/components/CourseCertModal.js
...
const CourseCertModal = ({ isOpen, onClose, nftItem, user }) => {
  ...
  const handleUpload = async () => {
    ...
    if (!user || !user.publicAddress || !user.email) {
      toast({
        title: "User information missing",
        description: "User address and email are required to upload the file.",
        status: "error",
        duration: 5000,
        isClosable: true,
      });
    ...
    try {
      const challengeData = {
        title: nftItem.title,
        tokens: nftItem.tokens,
        date: new Date().toISOString(),
        address: user.publicAddress,
        email: user.email,
      };
      await uploadImage(file, challengeData);
...
export default CourseCertModal;
